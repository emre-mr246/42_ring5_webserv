...
static void printListenAddresses(const std::vector<std::pair<std::string, int> > &listenOn)
{
...
static void printLocations(const std::vector<LocationConfig> &locations)
{
    ...
    while (it != locations.end())
    {
        ...
        std::cout << "[CONFIG]   Client max body size: " << (it->clientMaxBodySize + 1023) / 1024 << " KB" << std::endl;
        ...
        if (!it->cgiPass.first.empty())
            std::cout << "[CONFIG]   CGI pass: " << it->cgiPass.first << " -> " << it->cgiPass.second << std::endl;
        it++;
    }
}
